...
export const BILLING_MODE_UNIT = 'unit'
...
export function getBillingModeLabel(mode: string | null | undefined, t: (key: string) => string): string {
  switch (mode) {
    ...
    case BILLING_MODE_UNIT: return t('admin.usage.billingModeUnit')
...
export function getBillingModeBadgeClass(mode: string | null | undefined): string {
  switch (mode) {
    ...
    case BILLING_MODE_UNIT: return 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300'
